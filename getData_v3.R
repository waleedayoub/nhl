...
for(i in 1:nGames) {
  ...
  if (i==1) {
    ...
    ptsMember <- merge(ptsMember, t6, all=TRUE)
    ptsPlayer <- merge(ptsPlayer, ptsTable, all=TRUE)
  }
}
head(ptsPlayer)
head(ptsMember)
...
temp <- arrange(group_by(ptsPlayer, playername) %>% summarise(points = sum(tot_pts)), desc(points))
head(temp)
# ptsMember is the final table we need to add to and then plot
ggplot(ptsMember, aes(x=refdate, y=POINTS)) + geom_line(aes(colour=POOL_MEMBER)) + geom_point()
